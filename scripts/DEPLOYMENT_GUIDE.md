...
**What it does:**
1. Finds newest folder in `/Users/IUshakovsky/n8n_files/_tinyrests/posts`
...
scripts/deploy-post-auto.sh -g "Add article about wake windows"

# Copy + build Jekyll + commit + push
scripts/deploy-post-auto.sh -b -g "New post: wake windows"
```
...
Commit message will be auto-generated as: `blog post understanding wake windows guide`
...
```bash
cd /Users/IUshakovsky/Projects/TinyRests
git add _posts/2025-12-20-your-post.md assets/images/blog/your-image.jpg
...
```bash
alias deploy-tinyrests='cd /Users/IUshakovsky/Projects/TinyRests && scripts/deploy-post-auto.sh -g ""'
```
...
1. **Generate content** via n8n workflow → saves to `/Users/IUshakovsky/n8n_files/_tinyrests`
...
- `deploy-to-git.sh` auto-generates commit messages in format: "blog post [post-name]"
  - Example: `2025-12-20-understanding-wake-windows-guide.md` → `"blog post understanding wake windows guide"`
